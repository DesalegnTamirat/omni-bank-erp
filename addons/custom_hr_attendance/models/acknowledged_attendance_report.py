...
class AcknowledgedAttendanceReport(models.Model):
    ...
    reason_type = fields.Char(string="Acknowledged Type", readonly=True)

    reason_name = fields.Char(string="Attendance Reason", readonly=True)

    acknowledged_by = fields.Char(string="Acknowledged By", readonly=True)
        ...
        self.env.cr.execute("""
            CREATE OR REPLACE VIEW acknowledged_attendance_report AS (
                SELECT
                    ...
                    END AS reason_name,
                    COALESCE(part.name::text, '') AS acknowledged_by,
