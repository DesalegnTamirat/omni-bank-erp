...
class RecruitmentRequest(models.Model):
    ...
    # ── FR-09: Classification ─────────────────────────────────────────────
    request_type = fields.Selection(
        [("planned", "Planned"), ("unplanned", "Unplanned")],
        string="Request Type", required=True, default="planned", tracking=True,
        help="FR-09: Planned requests must link to an approved workforce plan."
    )

    # ── FR-08: Basic request info ─────────────────────────────────────────
    ...
    # ── FR-09: Workforce plan link (for Planned) ──────────────────────────
    # NOTE: this now correctly points at the real planning model
    # (planning.work.unit.manpower), not the placeholder "manpower.plan".
    workforce_plan_id = fields.Many2one(
        ...
        help="FR-09: Required for Planned requests. Only approved plans "
             "belonging to the selected Work Unit are selectable. "
             "Auto-filled when only one Approved plan exists for your Work Unit."
    )

    # ── FR-10: Justification (required for Unplanned) ─────────────────────
    justification = fields.Text(
        string="Business Justification",
        help="FR-10: Mandatory for Unplanned requests."
    )

    # ── FR-11: Job Information ────────────────────────────────────────────
    ...
    # ── FR-09: Plan headcount visibility (computed, not stored) ───────────
    ...
    # ─────────────────────────────────────────────────────────────────────
    # Plan headcount helpers (FR-09 quantity validation)
    # ─────────────────────────────────────────────────────────────────────
    ...
    def _check_unplanned_justification(self):
        """FR-10: Unplanned requests must have a business justification."""
        for rec in self:
            if rec.request_type == "unplanned" and not rec.justification:
                raise ValidationError(
                    _("FR-10: Business Justification is required for Unplanned recruitment requests.")
                )

    def _validate_against_workforce_plan(self):
        """FR-REC-006 / FR-REC-007 / FR-REC-008: automated validation run
        ...
        request (FR-REC-004). Validation only fires at the point the
        system routes the request onward for verification (FR-REC-005).

        Returns a list of (code, message) failure tuples. Empty list =
        passed. Each failure is specific per FR-REC-008 ("provide
        ...
        if not plan:
            failures.append((
                "no_plan",
                _("No Approved Workforce Plan is linked to this request."),
            ))
            return failures
        ...
        # FR-REC-006: Position ID must exist in the Approved Workforce Plan.
        ...
        # FR-REC-007: Requested Headcount must not exceed Approved Budgeted
    ...
    def _check_workforce_plan_or_raise(self):
        """FR-REC-008: prevent processing and surface specific reason(s)."""
        self.ensure_one()
    ...
    def _check_headcount_against_plan_on_save(self):
        ...
        plan/job/headcount can be saved as an incomplete Draft (FR-REC-004);
        once those are filled in, saving is blocked if headcount exceeds
        what's left on the plan (FR-REC-007/008)."""
        for rec in self:
    ...
    def _check_replacement(self):
        """FR-12: If replacement, employee or reason must be specified."""
        for rec in self:
            if rec.is_replacement and not rec.replaced_employee_id and not rec.replacement_reason:
                raise ValidationError(
                    _("FR-12: Please specify the employee being replaced or the reason for replacement.")
                )
    ...
    def action_submit(self):
        """FR-REC-004/005: User submits the request, which routes it to
        People Operations Management Directorate for verification.
        FR-REC-006/007/008: before routing, run the automated Position ID
    ...
    def action_approve(self):
        for rec in self:
            # Re-run the FR-REC-006/007 checks at approval time too, since
