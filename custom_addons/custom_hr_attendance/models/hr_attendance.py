...
class HrAttendance(models.Model):
    ...
    # ------------------------------------------------------------------
    # Public entry point for Attendance Dashboard client action
    # ------------------------------------------------------------------
    @api.model
    def get_attendance_dashboard(self, filter_type='today', specific_date=None, personal_period='this_month', personal_start=None, personal_end=None):
        ...
        """
        user_tz_name = self.env.user.tz or 'Africa/Addis_Ababa'
        try:
        ...
        present, late = self._dashboard_present_and_late(cr, start_utc, end_utc)
        leave = self._dashboard_on_leave(cr, target_date)
        ...
        return {
            ...
            'progress': self._dashboard_hourly_progress(cr, start_utc, end_utc, local_tz),
            'workunit': self._dashboard_workunit_breakdown(cr, start_utc, end_utc),
            'personal_summary': self._dashboard_personal_summary(local_tz, target_date, personal_period, personal_start, personal_end),
        }
    ...
    def _dashboard_present_and_late(self, cr, start_utc, end_utc):
        cr.execute("""
            SELECT
    ...
    def _dashboard_on_leave(self, cr, target_date):
        cr.execute("""
    ...
    def _dashboard_hourly_progress(self, cr, start_utc, end_utc, local_tz):
        cr.execute("""
    ...
    def _dashboard_workunit_breakdown(self, cr, start_utc, end_utc):
        cr.execute("""
            SELECT
                CASE
                    WHEN ou.work_unit_type = 'head_office' THEN 'head_office'
    ...
    def _dashboard_personal_summary(self, local_tz, target_date, date_range='this_month', start_date=None, end_date=None):
        user = self.env.user
