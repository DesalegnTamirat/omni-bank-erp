    ...
    def get_json_array(self):
        ...
        SELECT JSON_AGG(
            JSON_BUILD_OBJECT(
                'account', biv.account,
                'amount', biv.amount,
                'particular', biv.particular,
                'flag', biv.flag,
                'currency', biv.currency,
                'report_code', biv.report_code
            )
        )
        FROM bunna_increment_v biv
        WHERE 1=1
...
class increment_transfer_wizard(models.TransientModel):
    _name = 'increment.transfer.wizard'
