...
class EdsCurriculum(models.Model):
    """Curriculum / course design document (-019, ...013).

    Approval chain: L&D Quality Review -> Director PPDD -> CPCO Approval ().
    Revision returns the curriculum to the originating officer for update and
    resubmission with comment history (). Editing an approved curriculum
    ...
    # copy=True: a new version must deep-copy its modules ()
    ...
    review_sla_deadline = fields.Date(
        ...
        help='Computed from the curriculum SLA in settings (default 10 working days, ).')
    ...
    def action_submit_quality_review(self):
        """Draft -> L&D Quality Review ( step 1)."""
        for rec in self:
            if rec.state != 'draft':
            ...
            rec.message_post(body=_('Curriculum %s submitted to L&D Quality Review. SLA: %s ().')
    ...
    def action_cpco_approve(self):
        ...
        draft annual plan () and route e-learning needs to the LMS (Task 13)."""
        for rec in self:
            rec._require_manager()
            ...
            rec.message_post(body=_('Curriculum %s approved by CPCO ().') % rec.name)
            rec._on_curriculum_approved()

    def _on_curriculum_approved(self):
        """ + Task 13 contract hooks, defensive against modules not yet installed."""
        self.ensure_one()
    ...
    def action_request_revision(self):
        """Approved -> Revision (): return to the originating officer for update,
        then create a new version () archived from the old one."""
        for rec in self:
            rec._require_manager()
            if rec.state != 'approved':
                raise UserError(_('Only approved curricula can be sent for revision.'))
            if not rec.revision_comment:
                raise UserError(_('A revision comment is required before returning the curriculum ().'))
            # Version control (): copy to a new version, archive the previous
            ...
            rec.message_post(body=_('Curriculum %s returned for revision (). New version %s created.')
    ...
    def action_resubmit_revision(self):
        """Revision -> Quality Review: the officer updates and resubmits ()."""
        for rec in self:
...
class EdsCurriculumModule(models.Model):
    """One module inside a curriculum design ()."""
